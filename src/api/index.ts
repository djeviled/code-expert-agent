...
// Routes to managed session (MCP tools) or direct Claude fallback
// ────────────────────────────────────────────────────────────
app.post("/api/agent/message", async (c) => {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // ── PATH A: Managed Agents (MCP tools) ──
      if (anthropicSessionId) {
        try {
          ...
          for await (const event of eventStream) {
            ...
            } else if (e.type === "session.status_idle" || e.type === "session.end_turn") {
              break;
            } else if (e.type === "session.error") {
              send({ type: "error", message: e.error?.message || "Agent error" });
              controller.close();
              return;
            }
          }
          ...
          console.error("Managed session error, falling back:", err.message);
        }
      }

      // ── PATH B: Direct Claude fallback ──
      try {
        if (!ANTHROPIC_KEY) {
          send({ type: "chunk", text: "⚠️ ANTHROPIC_API_KEY is not configured." });
          send({ type: "done" });
          controller.close();
          return;
        }
...
app.post("/api/user/credentials", async (c) => {
  ...
  if (!provider || !access_token?.trim()) return c.json({ error: "provider and access_token required" }, 400);

  const VALID = ["github", "vercel", "supabase_url", "supabase_key", "anthropic"];
  if (!VALID.includes(provider)) return c.json({ error: "Invalid provider" }, 400);

  const { error: upsertErr } = await supabase
    .from("user_credentials")
    .upsert(
      { user_id: user.id, provider, access_token: access_token.trim(), metadata: metadata || null, updated_at: new Date().toISOString() },
...
app.delete("/api/user/credentials/:provider", async (c) => {
  ...
  const { data: { user }, error } = await supabase.auth.getUser(token);
  if (error || !user) return c.json({ error: "Unauthorized" }, 401);
  await supabase.from("user_credentials").delete()
    .eq("user_id", user.id).eq("provider", c.req.param("provider"));
